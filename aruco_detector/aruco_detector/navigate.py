''' This node is the integrated node for aruco makers detection as well as dwa navigation node '''
...
class DWANavigator(Node):
    ...
    def __init__(self):
        ...
        self.NAVIGATION_TIME = 600  # 5 minutes
    ...
    def object_callback(self, msg):
        ...
        if msg.data:
            ...
            if marker_id not in self.visited_markers:
                ...
                self.handle_markers(marker_id)
            else:
                self.get_logger().info(f"Marker {marker_id} detected again, no action taken.")
            self.previous_marker_id = marker_id
            self.previous_marker_position = self.robot_pose 
    
    def handle_markers(self, marker_id):
        if marker_id == 5:
            self.get_logger().info(f"Marker {marker_id} detected, spinning 360 degrees and pausing for 10 seconds")
            time.sleep(1)
            self.spin_360()
            time.sleep(1)

        elif marker_id == 2:
            self.get_logger().info(f"Marker {marker_id} detected, turn right twice")
            time.sleep(1)
            self.turn_right()
            time.sleep(1)
            self.turn_right()
            ...
            self.get_logger().info(f"Marker {marker_id} detected, sleeping for 10 seconds")
            time.sleep(10)

        elif marker_id == 3:
            self.get_logger().info(f"Marker {marker_id} detected, moving back 1 meter")
            time.sleep(1)
            self.move_back()

        elif marker_id == 6:
            self.get_logger().info(f"Marker {marker_id} detected, pointing to marker")
            time.sleep(1)
            self.move_forward_to_wall()
            time.sleep(10)

        elif marker_id == 7:
            self.get_logger().info(f"Marker {marker_id} detected, circle twice")
            self.move_in_circles()
            time.sleep(10)

        elif marker_id == 8:
            self.get_logger().info(f"Marker {marker_id} detected, go back to previous marker position and do the action for the same!")
            self.return_to_position(self.previous_marker_position)
            self.handle_markers(self.previous_marker_id)
    ...
    def move_forward_to_wall(self):
        ...
        while True:
            ...
            if self.laser_ranges:
                # Assuming front ranges are in the 0 to 30 and 330 to 360 degree range
                front_range = self.laser_ranges[:30] + self.laser_ranges[-30:]
                min_front_distance = min(front_range) - 0.27 # Subtract 0.3 meters
    ...
    def navigate(self):
        ...
        if current_time - self.start_time > self.NAVIGATION_TIME:
            ...
            self.return_to_position(self.start_pose)
    ...
    def return_to_position(self, position):
